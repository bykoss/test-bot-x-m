...
    def **init**(self, tipo: str, view):
        super().**init**(title=f"{'🟢 Rol a DAR' if tipo == 'dar' else '🔴 Rol a QUITAR'}")
        self.tipo = tipo
...
    def **init**(self, opciones, tipo, parent_view):
...
    def **init**(self, ctx, member: discord.Member):
...
    embed.add_field(name="🔗 Link", value=f"Descargar", inline=False)
    await ctx.send(embed=embed)
...
    embed = discord.Embed(title="🔗 Invitar", description=f"Clic aquí", color=discord.Color.blurple())
    await ctx.send(embed=embed)
...
    embed = discord.Embed(title=f"🔍 Buscar: {termino}", description=f"Haz clic para buscar en Google", color=discord.Color.blue())
    await ctx.send(embed=embed)
# ═════════════════════════════════════════════════════════════
# 🎁 SORTEOS Y ENCUESTAS
# ═════════════════════════════════════════════════════════════
...
    def **init**(self, autor, target, accion, info):
        super().**init**(timeout=60)
...
        activity=discord.Activity(type=discord.ActivityType.watching, name=f"{PREFIX}ayuda | System x&m ")
    )
@bot.event
